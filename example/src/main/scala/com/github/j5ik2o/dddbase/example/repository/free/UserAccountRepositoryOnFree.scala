...
class UserAccountRepositoryOnFree extends UserAccountRepository[ByFree] {
...
object UserAccountRepositoryOnFree {
  ...
  def eval[M[_]: Monad, A](repo: UserAccountRepository[M])(program: ByFree[A]): M[A] =
    program.foldMap(interpreter(repo))
